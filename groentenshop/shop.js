 ...
 * Loop through the cart items until either there are no more items or the specified
 * item is found.
 * 
...
function itemExists(productIndex) {
  ...
  if (nItems > 0) {
    var i = 0;
    /*while there is an item and the productindex differs from the requested one
     * and the loop hasn't reached the end of the array 
     */
    while (aShoppingCart[i] && aShoppingCart[i][1] !== productIndex && i < nItems) {
      i++;
    }
    /* If the loop stops before it has reached the end of the array then the product
     * was found. Return the item index(=current loop counter)
     */
    if (i < nItems) {
      index = i;
    }
...
function showShoppingCart() {
  ...
  //CHECK IF SHOPPINGCART CONTAINS ITEMS, CLEAR ITEMDIVS(=empty cart message)
...
function validateForm(frm) {
  ...
  if (nQuantity <= 0 || isNaN(nQuantity)) {
    errors.push ('Aantal moet cijfer groter dan 0 zijn');
  }
...
/**Show error messages generated by form validation
 * 
 * @param {type} errors
 * @returns {undefined}
 */
